...
typedef enum {
    ...
    DATA_SEND   = 0x02,
    ECHO_MSG    = 0x03,
    LED_CONTROL = 0x04  // New command for LED control
} Command;

volatile uint8_t ledControlFlag = 0;
...
/* USER CODE BEGIN PFP */
uint8_t UART4_rxBuffer[2] = {0};
uint8_t UART4_txBuffer[10] = {0};




#define HEALTH_MSG  0x1409
...
int main(void)
{
  ...
  //HAL_UART_Receive_IT(&huart4, UART4_rxBuffer, 8);
